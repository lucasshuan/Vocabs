...
- **Alcançar o alvo com uma mão só** — a escolha deixou de ser por ângulo e passou a exigir chegar ao disco, a ~150 dp do `+`. O raio de 44 dp perdoa a mira, mas em aparelho grande ninguém mediu quantos polegares chegam ao alvo da foto sem o telefone escorregar.
...
- **Voltar durante a gravação guarda, e nada na tela diz isso.** A regra do app é que nada é descartado sem a pessoa pedir, e o pedido tem um lugar só — o botão de descartar —, então o botão voltar encerra guardando. Quem apertou voltar esperando sair sem deixar rastro descobre pelo aviso de 5 s que a captura entrou.
...
- OCR e voz não seguem o curso. O modelo latino do ML Kit é o único empacotado, então capturar por foto em japonês devolve lixo ou nada, e a tela cai na edição manual sem explicar por quê. O aviso ficou mais difícil de dar desde que a captura virou gesto: não há mais folha onde escrever isso, e o lugar que sobrou é a linha da captura em Pendentes — depois do fato, e não antes.
- **"O idioma sai da fala" não sai.** O handoff do gesto trata a detecção de idioma como dada, e o app não tem detector nenhum: o que existe é a regra de fallback do próprio handoff — cai no curso aberto no hub — e ela virou o único caminho. A tela de gravação diz "vai para o inglês" em vez de prometer detecção, e a bandeira do aviso de 5 s mostra o mesmo curso. O dia em que houver detecção, são essas duas frases que mudam.
- **Foto e áudio só existem pelo gesto.** Sumiu a folha que listava os três modos lado a lado, e com ela o único caminho de captura por toque para foto e áudio. Quem nunca segurar o `+` não descobre que eles existem — o leque abre em 180 ms de pressão e a dica "arraste e solte no alvo" ensina de primeira, mas ninguém testou quantas pessoas chegam lá sozinhas. O convite do Início e o "capturar outra" do Guardado abrem a gaveta de texto, que é o modo mais provável, e não os outros dois.
